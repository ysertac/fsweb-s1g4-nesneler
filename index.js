...
function MenuElemaniOlustur(x, y, z) {
  return { isim: x, fiyat: y, kategori: z };
}
...
console.log(MenuElemaniOlustur("vişne Suyu", 20, "İçecekler"));
...
function DegerlendirmeEkle(x, y, z, w) {
  x.push({ isim: y, puan: z, geribildirim: w });
  return x;
}

DegerlendirmeEkle(degerlendirmeler, "Sertaç", 10, "Harika");
...
function AnahtardanDegerlendirmeAl(x, y) {
  return `${x[y].isim} isimli kişi ${x[y].puan} puan verdi ve şunları yazdı: ${x[y].geribildirim}`;
}
...
function SonDegerlendirmeyiAl(x) {
  return `${x[x.length - 1].isim} isimli kişi ${
    x[x.length - 1].puan
  } puan verdi ve şunları yazdı: ${x[x.length - 1].geribildirim}`;
}
